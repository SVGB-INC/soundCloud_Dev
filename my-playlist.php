...
$query = "SELECT * 
                ...
                ON fa.podcast = pd.pID
                WHERE user = '$userID'
                Limit 8
                ";
$sql = $pdo->query($query);
$sql->execute();
$likes_playlists = $sql->fetchAll(PDO::FETCH_ASSOC);


$stmtPopular = $pdo->query("SELECT upload_date,podcast_address,image_address,podcast_title,podcast_desc,podcast_likes,play_count, podcast_time,podcast_add_payment, podcast_charity, user_name, pID FROM podcast_details order by podcast_likes DESC, upload_date DESC LIMIT 10");
...
<body class="home-page">
      ...
      <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          ...
          <ul class="navbar-nav me-auto mb-2 mb-lg-0">
            ...
            <li class="nav-item mx-1">
              <a class="nav-link text-nowrap active" aria-current="page" href="./my-playlist.php">My Playlist</a>
            </li>
            <li class="nav-item mx-1">
  ...
  <form action="filters.php" method="post">
    ...
    <div class="container d-flex flex-wrap">
      ...
      <?php if($likes_playlists): ?>
      <div class="row mt-5">
        <div class="col ps-4 pb-3">
          <h3 class="d-inline">My Likes</h3>
        </div>
      </div>
      <div class="responsive-table res27 py-2">
      <?php endif;
      foreach($likes_playlists as $like_audio):
      ?>

<div class="rounded-bottom card card-1 d-flex position-relative top-0 start-0 flex-column">
          <?php if ($like_audio["podcast_add_payment"] == "paid") { ?>

            <!-- <span class="btn btn-warning btn-sm position-absolute" style="z-index: 10; top: 0.5rem; right:0.5rem">Paid</span> -->

          <?php } ?>
          <div class="imgContainer">
            <a id="podTitle" name="podTitle" style="display:none;" href=<?php echo str_replace(' ', '_', $like_audio["podcast_title"]) ?>></a>
            <a id="authorName" name="authName" style="display:none;" href=<?php echo str_replace(' ', '_', $like_audio["user_name"]) ?>></a>
            <a id="isCharity" name="charity" style="display:none;" href=<?php echo $like_audio["podcast_charity"] ?>></a>
            <a id="isPayment" name="payment" style="display:none;" href=<?php echo $like_audio["podcast_add_payment"] ?>></a>
            <a id="source-image" name="source-image" style="display:none;" href=<?php echo $like_audio["image_address"] ?>></a>
            <a id="source-audio" name="source-audio" style="display:none;" href=<?php echo $like_audio["podcast_address"] ?>></a>
            <img class="imgPod" src=<?php echo $like_audio["image_address"]; ?> />
            <!-- <div class="imageOverlay"></div> -->
          </div>
          <div class="content">
            <a class="audio_id" id="audio_id" name="audio_id" style="display:none;" href="<?= $like_audio["pID"] ?>" ></a>
            <a id="podTitle" name="podTitle" style="display:none;" href=<?php echo str_replace(' ', '_', $like_audio["podcast_title"]) ?>></a>
            <a id="authorName" name="authName" style="display:none;" href=<?php echo str_replace(' ', '_', $like_audio["user_name"]) ?>></a>
            <a id="isCharity" name="charity" style="display:none;" href=<?php echo $like_audio["podcast_charity"] ?>></a>
            <a id="isPayment" name="payment" style="display:none;" href=<?php echo $like_audio["podcast_add_payment"] ?>></a>
            ...
            <h3><?php echo $like_audio["podcast_title"]; ?></h3>
            <h5><?= $like_audio["user_name"] ?></h5>
            <p><?= $like_audio["podcast_desc"] ?></p>
            <div class="icons">
              <button class="playInformation" >
                <i class="fas fa-play clickPod"></i>
                <span><?= $like_audio["play_count"] ?></span>
              </button>
              <button class="addToPlayListInformation">
                ...
                <?php
                  ...
                  foreach($my_fvrt_audio as $myfvrt){
                    if($myfvrt['podcast'] == $like_audio['pID']){
                      $fvrt = true;
                    }
                  }
                  ?>
              <button class="heartIconCustomMade likeing-button clickAddToFavrt">
                  <i id="heartId_<?= $like_audio['pID']  ?>" class="<?php if($fvrt){ ?>fa-solid<?php } else{ ?>fa-regular<?php } ?> fa-heart"></i>
                  <span><?= $like_audio["podcast_likes"] ?></span>
                </button>
            </div>
      ...
      <div class="row mt-5">
        <div class="col ps-4 pb-3">
          <h3 class="d-inline">My History</h3>
        </div>
      </div>
      <div class="responsive-table res28 py-2">
      <?php
      ...
      <div class="row mt-5">
        <div class="col ps-4 pb-3">
          <h3 class="d-inline"><?= $playlist['title'] ?></h3>
        </div>
      </div>

      <div class="responsive-table res29 py-2">
        <?php foreach($playlist_data as $playlist_pod): ?>

        <div class="rounded-bottom card card-1 d-flex position-relative top-0 start-0 flex-column">
          <?php if ($playlist_pod['podcast_add_payment'] == "paid") { ?>

            <span class="btn btn-warning btn-sm position-absolute" style="z-index: 10; top: 0.5rem; right:0.5rem">Paid</span>

          <?php } ?>
          <div class="imgContainer">
            <a id="podTitle" name="podTitle" style="display:none;" href=<?php echo str_replace(' ', '_', $playlist_pod['title']) ?>></a>
            <a id="authorName" name="authName" style="display:none;" href=<?php echo str_replace(' ', '_', $playlist_pod['user_name']) ?>></a>

            <a id="isCharity" name="charity" style="display:none;" href=<?php echo $playlist_pod['podcast_charity'] ?>></a>
            <a id="isPayment" name="payment" style="display:none;" href=<?php echo $playlist_pod['podcast_add_payment'] ?>></a>
            <a id="source-image" name="source-image" style="display:none;" href=<?php echo $playlist_pod['image_address'] ?>></a>
            <a id="source-audio" name="source-audio" style="display:none;" href=<?php echo $playlist_pod['upload_date'] ?>></a>
            <img class="imgPod" src=<?php echo $playlist_pod['image_address']; ?> />
          </div>
          <div class="content">
          <a class="audio_id" id="audio_id" name="audio_id" style="display:none;" href="<?= $playlist_pod["pID"] ?>" ></a>
            ...
            <h3><?php echo $playlist_pod["podcast_title"]; ?></h3>
            <h5><?= $playlist_pod["user_name"] ?></h5>
            <p><?= $playlist_pod["podcast_desc"] ?></p>
            <div class="icons">
              <button class="playInformation" >
                <i class="fas fa-play clickPod"></i>
                <span><?= $playlist_pod["play_count"] ?></span>
              </button>
              <button class="addToPlayListInformation">
                <i class="fas fa-plus clickAddToPlayList"></i>
                ...
                <?php
                  $fvrt = false;
                  foreach($my_fvrt_audio as $myfvrt){
                    if($myfvrt['podcast'] == $playlist_pod['pID']){
                      $fvrt = true;
                    }
                  }
                  ?>
              <button class="heartIconCustomMade likeing-button clickAddToFavrt">
                  <i id="heartId_<?= $playlist_pod['pID']  ?>" class="<?php if($fvrt){ ?>fa-solid<?php } else{ ?>fa-regular<?php } ?> fa-heart"></i>
                  <span><?= $playlist_pod["podcast_likes"] ?></span>
                </button>
            </div>
          </div>
        </div>
