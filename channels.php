...
$sqlTags = "Select tags from channels where tags not like '%{%'";
...
$sql = $pdo->query($query);
$sql->execute();
$my_channels = $sql->fetchAll(PDO::FETCH_ASSOC);

// latest release
$query = "SELECT *
              FROM channels ORDER BY create_date DESC LIMIT 10;";
$sql = $pdo->query($query);
$sql->execute();
$latest_release_latest = $sql->fetchAll(PDO::FETCH_ASSOC);

//popular channels
$query = "SELECT * FROM channels as ch
INNER JOIN podcast_details as pd ON pd.podcast_channel = ch.title
AND pd.user_ID = ch.author
where ch.create_date > current_date - interval 7 day
ORDER BY ch.create_date, pd.podcast_likes DESC LIMIT 10;";
$sql = $pdo->query($query);
$sql->execute();
$latest_release = $sql->fetchAll(PDO::FETCH_ASSOC);

//trending channels
$query = "SELECT *, SUM(pd.podcast_airtime) as pd_airtime
              FROM channels as ch
              ...
              GROUP BY podcast_channel
                Order BY pd_airtime DESC
                LIMIT 10
                ";
$sql = $pdo->query($query);
$sql->execute();
$tranding_channels = $sql->fetchAll(PDO::FETCH_ASSOC);


//last 10 in genre channels
...
$query = "SELECT * FROM channels where genre like '%$randomGenre%' LIMIT 10;";
$sql = $pdo->query($query);
$sql->execute();
$randomGenres = $sql->fetchAll(PDO::FETCH_ASSOC);
...
<body class="home-page">
  ...
  <main class="container py-5">
    <form action="channel_filters.php" method="post">
    ...
    <div class="row mt-5">
      <div class="col ps-4 pb-3">
        <h3 class="d-inline">Top 10 Latest Channels</h3>
    ...
    <div class="responsive-table res1 py-2">
      ...
      <?php
      foreach ($latest_release_latest as $latest) {
      ?>
        <div class="rounded-bottom card card-1 d-flex position-relative top-0 start-0 flex-column">
          <a class="channel_name" id="channel_name" name="channel_name" style="display:none;" href="<?= $latest['title'] ?>"></a>
          <div class="imgContainer chanel_img">
            <img class="" src="./images/user_channel_pic/<?php echo $latest['img']; ?>" />
          </div>
          <div class="content">
            <h5 class="fs-6"><?php echo $latest["title"]; ?></h5>
          </div>
        </div>
      <?php } ?>
    ...
    <div class="row mt-5">
      <div class="col ps-4 pb-3">
        <h3 class="d-inline">My Channels</h3>
        <span class="ps-3 pb-2"><a href="#" class="pink-text">View More</a></span>
      </div>
    </div>
    <div class="responsive-table res1 py-2">
      ...
      ?>
        <div class="rounded-bottom card card-1 d-flex position-relative top-0 start-0 flex-column">
          <a class="channel_name" id="channel_name" name="channel_name" style="display:none;" href="<?= $my_channel['title'] ?>"></a>

          <div class="imgContainer chanel_img">
            <img class="" src="./images/user_channel_pic/<?php echo $my_channel['img']; ?>" />
          </div>
          <div class="content">
            <h5 class="fs-6"><?php echo $my_channel["title"]; ?></h5>
          </div>
        </div>
      <?php } ?>
    ...
    <div class="row mt-5">
      <div class="col ps-4 pb-3">
        <h3 class="d-inline">Trending Channels</h3>
        <span class="ps-3 pb-2"><a href="#" class="pink-text">View More</a></span>
      </div>
    </div>
    <div class="responsive-table res1 py-2">
      ...
      <?php
      foreach ($tranding_channels as $tradning) {
        if ($tradning['pd_airtime'] > 0) {
      ?>
          <div class="rounded-bottom card card-1 d-flex position-relative top-0 start-0 flex-column">
            <a class="channel_name" id="channel_name" name="channel_name" style="display:none;" href="<?= $tradning['title'] ?>"></a>
            <div class="imgContainer chanel_img">
              <img class="" src="./images/user_channel_pic/<?php echo $tradning['img']; ?>" />
            </div>
            <div class="content">
              <h5 class="fs-6"><?php echo $tradning["title"]; ?></h5>
            </div>
          </div>
      <?php }
      } ?>
    ...
    <div class="responsive-table res1 py-2">
      ...
      <?php
      foreach ($randomGenres as $genres) {
      ?>
        <div class="rounded-bottom card card-1 d-flex position-relative top-0 start-0 flex-column">
          <a class="channel_name" id="channel_name" name="channel_name" style="display:none;" href="<?= $genres['title'] ?>"></a>
          <div class="imgContainer chanel_img">
            <img class="" src="./images/user_channel_pic/<?php echo $genres['img']; ?>" />
          </div>
          <div class="content">
            <h5 class="fs-6"><?php echo $genres["title"]; ?></h5>
          </div>
        </div>
      <?php } ?>
...
<script>
  $('.chanel_img').click(function() {
    var channel_name = $(this).prevAll('#channel_name').first().attr('href')

    window.location.href = `./channel.php?channel=${channel_name}`
  }) 
</script>