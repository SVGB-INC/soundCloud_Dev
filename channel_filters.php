...
if (!empty($_POST['order'])) {
    if ($_POST['order'][0] === 'A to Z') {
        $chosenOrder = " Order by title ASC;";
    } elseif ($_POST['order'][0] === 'Z to A') {
        $chosenOrder = " Order by title DESC;";
    } elseif ($_POST['order'][0] === 'Upload Date ASC') {
        $chosenOrder = " Order by create_date ASC;";
    } elseif ($_POST['order'][0] === 'Upload Date DESC') {
        $chosenOrder = " Order by create_date DESC;";
    }
}
...
if (isset($chosenGenre) || isset($chosenTag) || isset($chosenOrder)) {
    ...
    if (isset($chosenGenre)) {
        $query .= " where genre like '%" . $chosenGenre . "%' ";
    }
    if (isset($chosenGenre) && isset($chosenTag)) {
        $query .= " and tags like '%" . $chosenTag . "%' ";
    } elseif (!isset($chosenGenre) && isset($chosenTag)) {
        $query .= " where tags like '%" . $chosenTag . "%' ";
    }
...
<head>
    ...
    <title>Chhogori | Channel Filters</title>
        ...
        <div class="responsive-table res1 py-2">
            <?php foreach ($searched_data as $data) { ?>
                <div class="rounded-bottom card card-1 d-flex position-relative top-0 start-0 flex-column">
                    <a class="channel_name" id="channel_name" name="channel_name" style="" href="<?= "channel.php?channel=" . $data['title'] ?>">
                        <div class="imgContainer chanel_img">
                            <img class="imgPod" src="./images/user_channel_pic/<?php echo $data['img']; ?>" />
                        </div>
                    </a>
                    <div class="content">
                        <h5 class="fs-6"><?php echo $data["title"]; ?></h5>
                    </div>
                </div>
