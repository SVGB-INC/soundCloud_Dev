...
// podcast suggest
$query = "SELECT * FROM podcast_details as pd
            LEFT JOIN user_listened_pod as lp
              ON pd.pID = lp.podcast
            WHERE lp.user = '1641574603130337380561d870cbaf58f932056165'
              AND pd.podcast_title = pd.podcast_title
            ORDER BY pd.pID DESC LIMIT 10
                ";
$sql = $pdo->query($query);
$sql->execute();
$suggest_podcasts = $sql->fetchAll(PDO::FETCH_ASSOC);

// Channel suggest
$query = "SELECT ch.* 
            FROM podcast_details pd
              LEFT JOIN user_listened_pod as lp
                ON pd.pID = lp.podcast
              LEFT JOIN channels as ch
                ON pd.podcast_genre LIKE '%' || ch.genre || '%'
              WHERE lp.user = '1641574603130337380561d870cbaf58f932056165'
              GROUP BY ch.title
              DESC LIMIT 10
                ";
$sql = $pdo->query($query);
$sql->execute();
$suggest_channels = $sql->fetchAll(PDO::FETCH_ASSOC);
...
$sql = $pdo->query($query);
$sql->execute();
$latest_podcasts = $sql->fetchAll(PDO::FETCH_ASSOC);

// poppular podcasts
$query = "SELECT *
              FROM podcast_details

                Order BY podcast_likes DESC
                LIMIT 10
                ";
  $sql = $pdo->query($query);
  $sql->execute();
  $poppular_podcast = $sql->fetchAll(PDO::FETCH_ASSOC);

// Most Listend podcasts
$query = "SELECT *
            FROM podcast_details
              Order BY podcast_airtime DESC
              LIMIT 10
                ";
  $sql = $pdo->query($query);
  $sql->execute();
...
$stmtList = $pdo->query("SELECT upload_date,podcast_address,image_address,podcast_title,podcast_desc,podcast_likes,play_count, podcast_time, podcast_add_payment, podcast_charity, user_name, pID  FROM podcast_details order by podcast_likes DESC, upload_date DESC LIMIT 25");
...
<head>
  ...
  <title>Chhogori | New & Popular</title>
</head>
      ...
      <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          ...
          <ul class="navbar-nav me-auto mb-2 mb-lg-0">
            ...
            <li class="nav-item mx-1">
              <a class="nav-link text-nowrap active" aria-current="page" href="./new_and_popular.php">New &amp; Popular</a>
            </li>
            <li class="nav-item mx-1">
  ...
  <main class="container py-5">
    <form action="">
      <div class="row g-3 align-items-center">
        ...
        <div class="col-auto">
          <select class="form-select" id="tagsFilter" aria-label="Default select example">
...
<?php
        ...
        <div class="col-auto">
          <select class="form-select" aria-label="Default select example">
            ...
            <option value="scisors">scisors</option>
          </select>
        </div>
        <!-- <div class="col-auto">
    ...
    <div class="row mt-5">
      <div class="col ps-4 pb-3">
        <h3 class="d-inline">New Messages</h3>
    ...
    <div class="responsive-table res2 py-2">
      ...
      <?php
      foreach($poppular_podcast as $podcast){
        if($podcast['podcast_likes'] > 0 ){
      ?>
...
<div class="rounded-bottom card card-1 d-flex position-relative top-0 start-0 flex-column">
    ...
    <div class="row mt-5">
      <div class="col ps-4 pb-3">
        <h3 class="d-inline">Most Listend</h3>
        <span class="ps-3 pb-2"><a href="#" class="pink-text">View More</a></span>
      </div>
    </div>
...
<!--Podcast suggest -->
    <div class="row mt-5">
      <div class="col ps-4 pb-3">
        <h3 class="d-inline">Suggested Messages</h3>
        <span class="ps-3 pb-2"><a href="#" class="pink-text">View More</a></span>
      </div>
    </div>
    ...
    <div class="responsive-table res23 py-2">
      ...
      <?php
      foreach($suggest_podcasts as $podcast){
        if($podcast['podcast_airtime']>0){
      ?>
...
<div class="row mt-5">
      <div class="col ps-4 pb-3">
        <h3 class="d-inline">Suggested Voices</h3>
        <span class="ps-3 pb-2"><a href="#" class="pink-text">View More</a></span>
      </div>
    </div>
<div class="responsive-table res1 py-2">
        <?php if($suggest_channels){ foreach($suggest_channels as $channel){ ?>
          <div class="rounded-bottom card card-1 d-flex position-relative top-0 start-0 flex-column">
