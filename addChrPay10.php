  ...
  try {
    ...
    if ($select_stmt->rowCount() > 0) {
      ...
      if ($stmt->execute([$_SESSION["user_name"]])) {
       ...
       $insert_stmt->execute([$audio_id, $authorNameNew, $_SESSION["user_name"], ($payDetails["podcast_payment"] * ($admin_details["add_share_admin"] / 100)), ($payDetails["podcast_payment"] * ($admin_details["add_share_creator"] / 100)), $_POST["charity-pay-ammount"], $date]);
       //insert payment details
      ...
      if ($insert_stmt->execute([$_SESSION["user_name"], $audio_id, $audio_id])) {
       ...
       $date = date("Y-m-d");
       $insert_stmt = $pdo->prepare("INSERT INTO podcast_payment_history(pID, author, user, admin_payment, creator_payment, charity, date) VALUES (?,?,?,?,?,?,?)"); //sql insert query
       $insert_stmt->execute([$audio_id, $authorNameNew, $_SESSION["user_name"], ($_POST["additional-pay-ammount"] * ($admin_details["add_share_admin"] / 100)), ($_POST["additional-pay-ammount"] * ($admin_details["add_share_creator"] / 100)), $_POST["charity-pay-ammount"], $date]);
       //insert payment details
      ...
      <div class="row pt-5">
        ...
        <div class="col-md-8 p-0">
          <form class="form py-5 px-md-4 px-2 px-lg-4 bg-opacity-75" name="signupform" method="post" style="min-height: 33rem;">
            <h1 class="text-center">Additional Payment and Optional Charity</h1>
              ...
              <div class="payment-select-container">
                ...
                <div class="form-check d-flex justify-content-between">
                  <div class="input d-flex justify-content-center align-items-center gap-3 text-md-nowrap">
                    <input class="form-check-input" type="checkbox" value="" checked disabled name="additional-pay" id="additional-pay">
                    <label class="form-check-label opacity-100" for="additional-pay">
                      Additional Payment
                    </label>
                    <label class=" form-check-label opacity-100">To <a class="link-primary"  href= <?php echo "profile.php?user=" . $authorNameNew; ?>><?php echo $authorNameNew; ?><?= " [ " . $authorPayPal . " ] "  ?></a></label>
                  </div>
                  <div class="input d-flex justify-content-center align-items-center gap-2" >
                    ...
                    <input class="form-control p-1" style="width: 5rem;" type="text" value=<?php echo $payDetails["podcast_payment"]; ?>  disabled name="additional-pay-ammount" id="additional-pay-ammount">
                  </div>
                </div>
                <div class="form-check d-flex justify-content-between">
                  <div class="input d-flex justify-content-center align-items-center gap-3 text-md-nowrap">
                    <input class="form-check-input" type="checkbox" value="" checked name="charity-pay" id="charity-pay">
                  ...
                  <div class="input d-flex justify-content-center align-items-center gap-2" >
                    <label class="ms-auto form-check-label" for="charity-pay">
                      Amount USD
                    </label>
                    <input class="form-control p-1" style="width: 5rem;" type="text" value=<?php echo $payDetails["podcast_charity"]; ?>  name="charity-pay-ammount" id="charity-pay-ammount">
                  </div>
                </div>
              </div>
...
let additional_payment = document.getElementById('additional-pay-ammount').value;

let charity_payment_element = document.getElementById('charity-pay-ammount');
let charity_payment = charity_payment_element.value;

let toal_payment =  parseFloat(additional_payment)  + parseFloat(charity_payment) 

charity_payment_element.addEventListener('keyup', function(e){
  charity_payment = e.target.value;
          ...
          return actions.order.capture().then(function(orderData) {
            ...
            setTimeout(() => {
              $.ajax({
                  ...
                  method: "POST",
                  data: {
                    additional_payment, charity_payment, charity_acc, auth_acc
                  },
